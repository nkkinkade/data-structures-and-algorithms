...
const createServer = () => {
  app.get('/hello', (request, response) => response.send('Hello! Hi there!'))
  app.get('/aboutMe', (request, response) => response.send('My name is Nathan Kinkade, a student at DeltaV.'))
  app.get('/favoriteFoods', (request, response) => response.send(['taco', 'pizza', 'cheeseburger']))
...
const totalSum = (input) => {
  ...
  let x = input.map(i => {return i.reduce(add)});
  return x.reduce(add);
...
const divisibleByFiveTwoToThePower = (input) => {
  let divide = input.map(i=> {return i.filter(x=> {return (typeof x === 'number' && x%5 < 1)})});
  let result = divide.map(i=> {return i.map(x=> {return Math.pow(2,x)})})
  return result
};
...
let findMaleAndFemale = (data) => {
  let filter = data.filter(x => x.gender === 'male' || x.gender === 'female');
  let persons = filter.map(x => x.name);
  let reducer = (names, name) => names + ' and ' + name;
