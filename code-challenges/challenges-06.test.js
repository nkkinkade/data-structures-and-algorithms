...
const totalCharacters = (arr) => {
  ...
  for(let i = 0; i < arr.length; i++){
    result = result + 1;
    if(typeof(arr[i].spouse) === "string"){
      result = result + 1;
    }if(arr[i].children.length > 0){
      result = result + arr[i].children.length;
    }
...
const houseSurvivors = (arr) => {
  const survivors = [];
  for(let i = 0;i<arr.length;i++){
    for(let x = 0;x<deceasedSpouses.length;x++){
      if(arr[i].spouse === deceasedSpouses[x] ){
         arr[i].spouse = null;
